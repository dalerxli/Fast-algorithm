...
extern inline const PetscErrorCode ScatSMatMultFFT(Mat,Vec,Vec);
...
Vec 		RealSpaceCube,FreqSpaceCube,ReconstructCube,FFTPaddedGreen;
Mat 		FFTMatrix;
//MPI:
int           rank;
int           size;
...
{
  Vec            x,y,z,rC,u0,u0P,u0IE,Distance,nParticlePerCube;     /* approx solution, RHS, solution-difference */
  //Vec            r,rP,rIE;                                         /* residual */
  Mat            A,B,IE,AFFT;           		                    /* linear system matrix */
  KSP            ksp,kspP,kspIE,kspFFT;      	                           /* linear solver context */
  ...
  Vec		   FFTx;
  //Vec		   FFTr;
  PetscBool      standard = PETSC_FALSE;
  ...
  ierr = PetscOptionsGetBool(PETSC_NULL,"-standard",&standard,PETSC_NULL);CHKERRQ(ierr);
  ...
  ierr = MatCreateFFT(PETSC_COMM_WORLD,DIM,dim,MATFFTW,&FFTMatrix);CHKERRQ(ierr);
  ierr = MatGetVecsFFTW(FFTMatrix,&RealSpaceCube,&FreqSpaceCube,&ReconstructCube);CHKERRQ(ierr);
  ierr = PetscObjectSetName((PetscObject) RealSpaceCube, "Real space vector");CHKERRQ(ierr);
  ierr = PetscObjectSetName((PetscObject) FreqSpaceCube, "Frequency space vector");CHKERRQ(ierr);
  ierr = PetscObjectSetName((PetscObject) ReconstructCube, "Reconstructed vector");CHKERRQ(ierr);
...
/*
  //Testing:
  VecDuplicate(FFTx,&FFTr);
  ierr = VecSet(FFTx,1.0);CHKERRQ(ierr);
  MatMult(AFFT,FFTx,FFTr);
  //VecView(FFTr,PETSC_VIEWER_STDOUT_WORLD);
*/
  ...
  ierr = VecDestroy(&FreqSpaceCube);CHKERRQ(ierr);
  ierr = VecDestroy(&ReconstructCube);CHKERRQ(ierr);
  //ierr = VecDestroy(&u0);CHKERRQ(ierr);
  //ierr = VecDestroy(&FFTr);CHKERRQ(ierr);
  ierr = MatDestroy(&FFTMatrix);CHKERRQ(ierr); 
...
if(standard)
{
  ...
  ierr = MatCreateShell(PETSC_COMM_WORLD,PETSC_DECIDE,PETSC_DECIDE,M,M,(void*)0,&A);CHKERRQ(ierr);
...
/*
  //ierr = VecCreateMPI(PETSC_COMM_WORLD,PETSC_DECIDE,M,&u0);CHKERRQ(ierr);  //Better create vector thread
  ierr = VecCreate(PETSC_COMM_WORLD,&u0);CHKERRQ(ierr);
  ierr = VecSetSizes(u0,PETSC_DECIDE,M);CHKERRQ(ierr);
  ierr = VecSetFromOptions(u0);CHKERRQ(ierr);
*/
...
/*
  ...
  VecAXPY(FFTr,-1.0,u0);  
  //VecView(FFTr,PETSC_VIEWER_STDOUT_WORLD);
  VecNorm(FFTr,NORMTYPE,&norm);
  PetscPrintf(PETSC_COMM_WORLD,"\nDifference between Standard Mult vs Convolution:\t%G\n",norm);
  VecDestroy(&FFTr);
*/
/*
  VecGetOwnershipRange(u0,&Istart,&Iend);
...
*/
  ...
  */
  ...
  //ierr = VecNorm(r,NORMTYPE,&norm);CHKERRQ(ierr);
...
//----------------------------------------SOLVING THE INTEGRAL EQUATION-------------------------------------------------------
  ...
  if(standard)
  {
    //Compare x & FFTx:
    VecAXPY(x,-1.0,FFTx);
    ierr = VecNorm(x,NORMTYPE,&norm);CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD,"(Original system)   vs (Nonconvolution):\t%G\n",norm);CHKERRQ(ierr);
  }
...
{
  PetscInt       n,s,t,j,segment=1028;
  PetscInt       p[segment];
  PetscScalar    tmp,v[segment];
      ...
      for(t = xstart; t < N2; t++)
      {
	   for(j = 0; j < segment; j++)
	 	...
	 	p[j] = t+j*N1;
          ...
          VecGetValues(xx,segment,p,v);
          for(j = 0; j < segment; j++)
          {
          	tmp += Scat3DS.CoefMatFast(s,p[j])*v[j];
	   }
      }
      for(t = xstart + segment*N1; t < xend; t++)
      {
	   VecGetValues(xx,1,&t,&v[0]);
          tmp += Scat3DS.CoefMatFast(s,t)*v[0];
      }
...
inline const PetscErrorCode ScatSMatMultFFT(Mat mat,Vec xx,Vec yy)
{
  PetscInt	n;
  ...
  MatMult(FFTMatrix,RealSpaceCube,FreqSpaceCube);
  PetscPrintf(PETSC_COMM_WORLD,".");

  //Matrix-vector multiplication in Fourier space
  VecPointwiseMult(FreqSpaceCube,FreqSpaceCube,FFTPaddedGreen);
  PetscPrintf(PETSC_COMM_WORLD,".");

  // Apply FFTW_BACKWARD 
  MatMultTranspose(FFTMatrix,FreqSpaceCube,ReconstructCube);
  PetscPrintf(PETSC_COMM_WORLD,".");

  VecCopyN(ReconstructCube,yy,n);
  PetscPrintf(PETSC_COMM_WORLD,".\n");
...
PetscErrorCode FFTPaddedGreenCube(PetscInt dim[])
{
    ...
    for(m3=slap1;m3<slap;m3++)
    {
	     ...
	     for(m1=0;m1<N;m1++)
	     {		
 		  xa[m1][m2][m3] = Scat3DS.GreenCube[m1][m2][m3];
	     }
         }
    }
    ...
    for(m3=slap1;m3<slap;m3++)    
    {
	     ...
	     {				
 		  xa[m1][m2][m3] = Scat3DS.GreenCube[2*N-m1-1][m2][m3];		
	     }
         }
    }
    slap = (N>slap2)?(slap2):(N);
    for(m3=slap1;m3<slap;m3++)
    {
	     ...
	     for(m1=0;m1<N;m1++)
	     {				
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m1][2*N-m2-1][m3];					
	     }
	     for(m1=N;m1<dim[0];m1++)
	     {				
		  m = 2*N-m1-1;
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m][2*N-m2-1][m3];					
	     }
         }
    }
    ...
    for(m3=slap;m3<slap2;m3++)
    {
       for(m2=0;m2<N;m2++)
       {
	     for(m1=0;m1<N;m1++)
	     {				
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m1][m2][2*N-m3-1];					
	     }
	     for(m1=N;m1<dim[0];m1++)
	     {				
		  m = 2*N-m1-1;
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m][m2][2*N-m3-1];					
	     }
       }
       for(m2=N;m2<dim[1];m2++)
       {
	     ...
	     for(m1=0;m1<N;m1++)
	     {				
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m1][mm][2*N-m3-1];					
	     }
	     for(m1=N;m1<dim[0];m1++)
	     {				
		  m = 2*N-m1-1;
	    	  xa[m1][m2][m3] = Scat3DS.GreenCube[m][mm][2*N-m3-1];					
	     }
       }
    }
