...
inline PetscErrorCode VecCopyN3(Vec xx,Vec yy,PetscInt N)
{
    ...
    if(Istart>=0 && Istart<N)
    {
       ...
       PetscMemoryGetCurrentUsage(&mem);PetscPrintf(PETSC_COMM_WORLD,"Copy internal vector:\tMem used: %G \t\n",mem);
       PetscFree(pos);      
    }
    VecAssemblyBegin(yy);
    VecAssemblyEnd(yy);
...
#undef __FUNCT__
#define __FUNCT__ "VecCopyN3b"
inline PetscErrorCode VecCopyN3b(Vec xx,Vec yy,PetscInt N)
{
    PetscInt    j;
    PetscInt    Istart,Iend,*pos;
    PetscScalar *xa;
