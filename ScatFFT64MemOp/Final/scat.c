...
//----------------------------------------SOLVING THE INTEGRAL EQUATION-------------------------------------------------------
  ...
  ierr = VecGetOwnershipRange(nParticlePerCube,&Istart,&Iend);CHKERRQ(ierr);
  ierr = VecGetArray(nParticlePerCube,&xa);CHKERRQ(ierr);
  for(PetscInt s = Istart; s <Iend ; s++)
  {
      if(xa[s-Istart] == 0)
	 xa[s-Istart] = 1;
  }
  ierr = VecRestoreArray(nParticlePerCube,&xa);CHKERRQ(ierr);
  ierr = VecPointwiseDivide(Distance,Distance,nParticlePerCube);CHKERRQ(ierr);
  ierr = VecNorm(Distance,NORMTYPE,&norm);CHKERRQ(ierr);
