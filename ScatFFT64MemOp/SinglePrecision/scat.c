...
{
  Vec            y,z,u0,u0P,u0IE,Distance,nParticlePerCube;       //approx solution, RHS, solution-difference 
  ...
  KSPGetType(kspFFT,&ksptype);
  KSPGetTolerances(kspFFT,&Tol,&aTol,&norm,&its);
  PetscPrintf(PETSC_COMM_WORLD,"\nComputing:\nTolerance:\t%G\nMethod:\t\t%s\n",Tol,ksptype);
...
inline PetscErrorCode ScatSMatMultFFT(Mat mat,Vec xx,Vec yy)
{
  ...
  PetscPrintf(PETSC_COMM_WORLD,"Zero vector:\t\t");
  GetTime(0);

  //VecCopyN(xx,RealSpaceCube,N);  //will use more memory to create new local variables. So do it here:
    ...
    VecGetArray(xx,&xa);
    VecSetValues(RealSpaceCube,Iend-Istart,pos,xa,INSERT_VALUES);
    VecRestoreArray(xx,&xa);     
    PetscFree(pos);     
    PetscPrintf(PETSC_COMM_WORLD,"Copy vector:\t\t");
    GetTime(0);
    VecAssemblyBegin(RealSpaceCube);
    VecAssemblyEnd(RealSpaceCube);
    PetscPrintf(PETSC_COMM_WORLD,"Assembly vector:\t");
    GetTime(0);

  // Apply FFTW_FORWARD for xx:
  fftwf_execute(fplan);
  PetscPrintf(PETSC_COMM_WORLD,"Apply FFT:\t\t");
  GetTime(0);

  //Matrix-vector multiplication in Fourier space
  VecPointwiseMult(RealSpaceCube,RealSpaceCube,FFTPaddedGreen);
  PetscPrintf(PETSC_COMM_WORLD,"Multiply vectors:\t");
  GetTime(0);

  // Apply FFTW_BACKWARD 
  fftwf_execute(bplan);
  PetscPrintf(PETSC_COMM_WORLD,"Apply iFFT:\t\t");
  GetTime(0);

  //VecCopyN(RealSpaceCube,yy,N);   //will use more memory to create new local variables. So do it here:
    ...
    {       
       ...
       VecGetArray(RealSpaceCube,&xa);
       VecSetValues(yy,Iend-Istart,pos,xa,INSERT_VALUES);
       VecRestoreArray(RealSpaceCube,&xa);     
       PetscFree(pos);     
    }
    PetscPrintf(PETSC_COMM_WORLD,"Copy results:\t\t");
    GetTime(0);
    VecAssemblyBegin(yy);
    VecAssemblyEnd(yy);
    PetscPrintf(PETSC_COMM_WORLD,"Assembly vector:\t");
